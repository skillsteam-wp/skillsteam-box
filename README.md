...
|---------------------------|----------------------------------------------------|-------------------------------------------------------------------------------------------------|
| NEXT_PUBLIC_APP_ENV       | PROD                                               | Среда запуска приложения                                                                        |
| NEXT_PUBLIC_FRONT_URL     | http://localhost:3000                              | Публичный адрес приложения                                                                      |
| NEXT_PUBLIC_BFF_PATH      | /api                                               | Точка входа в BFF слой для проксирования запросов                                               |
| NEXT_PUBLIC_SENTRY_DSN    | http://sentry-dsn                                  | DSN ключ для запросов к Sentry                                                                  |
| NEXT_PUBLIC_DADATA_APIKEY | Api key                                            | публичный апи ключ дадаты для запросов с браузера                                               |
...
| SMTP_HOSTNAME             | skillsteam.pro                                     | Хост почтового ящика отправителя                                                                |
...
| FRONT_HOST                | front                                              | Хост сервиса фронта (для запуска фронт-сервера)                                                 | 
| FRONT_PORT                | 3000                                               | Порт сервиса фронта (для запуска фронт-сервера)                                                 | 
...
После установки и запуска перейдите на страницу приложения в браузере и пройдите процесс регистрации.